...
} from '../../models/transactionReference';
...
export class TransactionReferenceService {
  public async addTransactionReference(
    ...
    saveCard = false,
    reference?: string
  ): Promise<ITransactionReference> {
    reference =
      reference || TransactionReferenceService.generateReferenceNumber();
    return await TransactionReference.findOneAndUpdate(
      { userId, reason, itemId },
      { amount, itemId, role, reason, reference, saveCard, used: false },
      getUpdateOptions()
    )
  ...
  public async updateTransactionReference(
    reference: string,
    body: ITransactionReference
  ): Promise<ITransactionReference> {
    stripUpdateFields(body);
    return await TransactionReference.findOneAndUpdate({ reference }, body)
      .lean<ITransactionReference>()
      .exec();
  }
  ...
  public async getTransactionReference(
    ...
    const transactionReference: ITransactionReference =
      await TransactionReference.findOne({ reference })
        .lean<ITransactionReference>()
        .exec();
